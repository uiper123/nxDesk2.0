...
async fn start_test_server() -> (u16, Arc<RelayServer>) {
    ...
    let server_with_port = Arc::new(RelayServer::new(
        server_config,
...
async fn test_client_agent_relay_connection() {
    ...
    // 1. Connect agent
    let mut agent_socket = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();
    let agent_handshake =
        r#"{"role":"agent","token":"agent_secret","session_id":"session-test-1"}"#;
    agent_socket
        .write_all(agent_handshake.as_bytes())
        .await
        .unwrap();
    ...
    // 2. Connect client
    let mut client_socket = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();
    let client_handshake =
        r#"{"role":"client","token":"client_secret","session_id":"session-test-1"}"#;
    client_socket
        .write_all(client_handshake.as_bytes())
        .await
        .unwrap();
    ...
    agent_reader
        .read_exact(&mut incoming_agent_payload)
        .await
        .unwrap();
...
async fn test_heartbeat_timeout() {
    ...
    let mut agent_socket = TcpStream::connect(format!("127.0.0.1:{}", port))
        ...
        .unwrap();
    let agent_handshake =
        r#"{"role":"agent","token":"agent_secret","session_id":"session-heartbeat-1"}"#;
    agent_socket
        .write_all(agent_handshake.as_bytes())
        .await
        .unwrap();
...
async fn test_reconnect_behavior() {
    ...
    // Connect agent first time
    {
        let mut agent_socket = TcpStream::connect(format!("127.0.0.1:{}", port))
            ...
            .unwrap();
        let agent_handshake =
            r#"{"role":"agent","token":"agent_secret","session_id":"session-reconnect-1"}"#;
        agent_socket
            .write_all(agent_handshake.as_bytes())
            .await
            .unwrap();
    ...
    let mut agent_socket_2 = TcpStream::connect(format!("127.0.0.1:{}", port))
        .await
        .unwrap();
    let agent_handshake_2 =
        r#"{"role":"agent","token":"agent_secret","session_id":"session-reconnect-1"}"#;
    agent_socket_2
        .write_all(agent_handshake_2.as_bytes())
        .await
        .unwrap();
