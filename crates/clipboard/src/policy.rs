use crate::traits::{ClipboardContent, ClipboardPolicy};
...
impl ClipboardPolicy for ConfigurableClipboardPolicy {
    fn is_allowed(&self, content: &ClipboardContent) -> bool {
        match content {
            ClipboardContent::Text(t) => self.allow_text && t.len() <= self.max_text_len,
            ClipboardContent::Image(img) => self.allow_image && img.len() <= self.max_image_size,
            ClipboardContent::Html(html) => self.allow_html && html.len() <= self.max_html_len,
        }
    }
